...
int nmeta; // Number of meta blocks (boot, sb, nlog, inode, bitmap) // 非資料區塊數量 (描述區塊)，前面加 2 是因為有啟動區塊和超級區塊
...
char zeroes[BSIZE]; // 
uint freeinode = 1;
uint freeblock; // 自由區塊的起頭// the first free block that we can allocate (資料區塊的第一塊為目前的 freeblock) 
...
int fs_init() {
  ...
  assert((BSIZE % sizeof(struct dirent)) == 0); // 區塊大小必須是 dirent 大小的倍數
	...
	nmeta = 2 + nlog + ninodeblocks + nbitmap; // Number of meta blocks (boot, sb, nlog, inode, bitmap) // 非資料區塊數量 (描述區塊)，前面加 2 是因為有啟動區塊和超級區塊
	nblocks = FSSIZE - nmeta;  // Number of data blocks // 資料區塊數量
	freeblock = nmeta;
  // 超級區塊設定
  sb.magic = FSMAGIC;             // 魔數
...
int fs_build(int fcount, char *fnames[]) {
  ...
  for(i = 0; i < fcount; i++){ // 對於參數列中的每個檔案，都加入到系統根目錄中
    // get rid of "user/"
    char *shortname;
    if(strncmp(fnames[i], "user/", 5) == 0) // 只取 user/ 下檔案的檔名
      shortname = fnames[i] + 5;
    else
      shortname = fnames[i];
    
    assert(strchr(shortname, '/') == 0); // assert(index(shortname, '/') == 0); 

    if((fd = fopen(fnames[i], "rb+")) == NULL){ // ccc: if((fd = open(argv[i], 0)) < 0){
      perror(fnames[i]);
      exit(1);
    }
...
ushort xshort(ushort x) // short 排列正規化
{
  ushort y;
  uchar *a = (uchar*)&y;
  a[0] = x;
